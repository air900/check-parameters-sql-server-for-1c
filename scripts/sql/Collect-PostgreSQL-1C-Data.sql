...
SELECT 28, 'wal',
    'commit_delay',
    'Задержка группового коммита',
    current_setting('commit_delay') || ' us',
    current_setting('commit_delay')

UNION ALL
SELECT 29, 'wal',
    'commit_siblings',
    'Порог группового коммита',
    current_setting('commit_siblings'),
    current_setting('commit_siblings')
...
SELECT 34, 'planner',
    ...
    CASE current_setting('geqo') WHEN 'on' THEN 'true' ELSE 'false' END
...
UNION ALL
SELECT 46, 'connections',
    'ssl',
    'Шифрование подключений',
    current_setting('ssl'),
    CASE current_setting('ssl') WHEN 'on' THEN 'true' ELSE 'false' END
...
SELECT 62, 'bgwriter',
    ...
    'Множитель фоновой записи',
    current_setting('bgwriter_lru_multiplier'),
    current_setting('bgwriter_lru_multiplier')
...
SELECT 73, 'compat_1c',
    ...
    'Отложенные temp-таблицы в памяти',
    COALESCE(current_setting('enable_temp_rd_buffers', true), 'n/a'),
    COALESCE(current_setting('enable_temp_rd_buffers', true), '')
...
SELECT 91, 'limits',
    'temp_tablespaces',
    'Табличные пространства для temp',
    COALESCE(NULLIF(current_setting('temp_tablespaces'), ''), '(default)'),
...
SELECT 96, 'archiving',
    ...
    CASE
        WHEN current_setting('archive_command') = '' THEN '(not configured)'
        ELSE left(current_setting('archive_command'), 80)
    END,
...
SELECT 97, 'archiving',
    'archive_timeout',
    'Принудительное переключение WAL',
    CASE current_setting('archive_timeout')
        WHEN '0' THEN 'off'
        ELSE current_setting('archive_timeout') || ' s'
    END,
...
-- pg_stat_statements убран: FROM несуществующей таблицы ломает весь UNION ALL
-- (PostgreSQL проверяет таблицы на этапе парсинга, не выполнения)
...
FROM (
    ...
    WHERE name NOT IN (
        ...
        'checkpoint_completion_target', 'wal_buffers', 'wal_level', 'commit_delay', 'commit_siblings',
        -- Планировщик
        ...
        'geqo_threshold', 'geqo', 'default_statistics_target', 'jit', 'max_parallel_workers_per_gather',
        -- Подключения
        'max_connections', 'idle_in_transaction_session_timeout', 'max_locks_per_transaction', 'ssl',
        -- Autovacuum / bgwriter
        ...
        -- Лимиты
        'max_files_per_process', 'temp_tablespaces',
        -- Архивирование
...
-- ----------------------------------------------------------------------------
-- Платформа 1С на PG использует те же префиксы что и на MSSQL, но lowercase
-- (PG case-folds unquoted identifiers): _reference<NNN>, _document<NNN>,
-- _accumreg<NNN>, _accumreg<NNN>totals, _inforeg<NNN>, _accreg<NNN>, etc.
--
-- Зачем: размер БД физический не информативен (compression, free pages,
-- toast). Количество строк по 1С-сущностям — meaningful baseline для
-- сравнения "характера нагрузки" двух 1С-баз.
--
-- Источник: docs/research/mssql-config-inventory-coverage-2026-04-27.md.
-- Стоимость: pg_stat_user_tables.n_live_tup — оценка от последнего analyze
-- (мгновенно). pg_total_relation_size — sum'ит heap+indexes+toast (доли сек
-- даже на 1ТБ-БД — это metadata read).
--
...
SELECT
    ...
    'workload' AS "Section",
    '_1c_' || category || '_' || current_database() AS "Key",
    'Профиль 1С — ' || category || ' в БД: ' || current_database() AS "Label",
    tables_count || ' таблиц / ' || total_rows || ' строк / ' || size_mb || ' MB' AS "Display",
    ('{"db":"' || current_database() ||
      '","category":"' || category ||
      '","tables_count":' || tables_count ||
      ',"total_rows":' || total_rows ||
      ',"size_mb":' || size_mb || '}') AS "Value"
FROM (
    SELECT
        category,
        COUNT(*)::bigint AS tables_count,
        COALESCE(SUM(n_live_tup), 0)::bigint AS total_rows,
        COALESCE(SUM(rel_size_bytes) / 1024 / 1024, 0)::bigint AS size_mb
    FROM (
        SELECT
            c.relname,
            COALESCE(s.n_live_tup, 0)::bigint AS n_live_tup,
            pg_total_relation_size(c.oid)::bigint AS rel_size_bytes,
            CASE
                -- Табличные части (lowercase в PG): _<тип>NNN_vtMMM.
                -- Проверяем ПЕРВЫМ, иначе попадёт в parent type.
                WHEN c.relname ~ '_vt[0-9]+'                 THEN 'tabular_section'

                -- Справочники / документы
                WHEN c.relname ~ '^_reference[0-9]+'         THEN 'reference'
                WHEN c.relname ~ '^_documentjournal[0-9]+'   THEN 'document_journal'
                WHEN c.relname ~ '^_document[0-9]+'          THEN 'document'

                -- Регистры накопления (1С использует Rg, не Reg).
                -- Порядок: AccumRgT/AccumRgOpt раньше общего AccumRg.
                WHEN c.relname ~ '^_accumrgt[0-9]+'          THEN 'accumreg_totals'
                WHEN c.relname ~ '^_accumrgopt[0-9]+'        THEN 'accumreg_totals'
                WHEN c.relname ~ '^_accumrgchngr[0-9]+'      THEN 'accumreg_movements'
                WHEN c.relname ~ '^_accumrgdl[0-9]+'         THEN 'accumreg_movements'
                WHEN c.relname ~ '^_accumrg[0-9]+'           THEN 'accumreg_movements'

                -- Регистры сведений (срезы + change registration внутри parent)
                WHEN c.relname ~ '^_inforgchngr[0-9]+'       THEN 'inforeg'
                WHEN c.relname ~ '^_inforgsl[0-9]+'          THEN 'inforeg'
                WHEN c.relname ~ '^_inforgsf[0-9]+'          THEN 'inforeg'
                WHEN c.relname ~ '^_inforg[0-9]+'            THEN 'inforeg'

                -- Регистры бухгалтерии (extension data, аналитические итоги внутри)
                WHEN c.relname ~ '^_accrgchngr[0-9]+'        THEN 'accreg'
                WHEN c.relname ~ '^_accrged[0-9]+'           THEN 'accreg'
                WHEN c.relname ~ '^_accrgat[0-9]+'           THEN 'accreg'
                WHEN c.relname ~ '^_accrgct[0-9]+'           THEN 'accreg'
                WHEN c.relname ~ '^_accrg[0-9]+'             THEN 'accreg'

                -- Регистры расчёта
                WHEN c.relname ~ '^_calcrgchngr[0-9]+'       THEN 'calcreg'
                WHEN c.relname ~ '^_calcrgactper[0-9]+'      THEN 'calcreg'
                WHEN c.relname ~ '^_calcrgrecalc[0-9]+'      THEN 'calcreg'
                WHEN c.relname ~ '^_calcrgdr[0-9]+'          THEN 'calcreg'
                WHEN c.relname ~ '^_calcrg[0-9]+'            THEN 'calcreg'

                WHEN c.relname ~ '^_chrc[0-9]+'              THEN 'chrc'
                WHEN c.relname ~ '^_acc[0-9]+'               THEN 'accchrt'  -- after accrg/accumrg
                WHEN c.relname ~ '^_enum[0-9]+'              THEN 'enum'
                WHEN c.relname ~ '^_const[0-9]+'             THEN 'const'

                -- Узлы планов обмена / последовательности / задачи / бизнес-процессы
                WHEN c.relname ~ '^_node'                    THEN 'platform_other'
                WHEN c.relname ~ '^_sequence'                THEN 'platform_other'
                WHEN c.relname ~ '^_task'                    THEN 'platform_other'
                WHEN c.relname ~ '^_bproc'                   THEN 'platform_other'
                WHEN c.relname ~ '^_bprg'                    THEN 'platform_other'

                WHEN c.relname ~ '^_'                        THEN 'platform_other'
                ELSE 'non_1c'
            END AS category
        FROM pg_class c
        LEFT JOIN pg_stat_user_tables s ON s.relid = c.oid
        WHERE c.relkind = 'r'                           -- ordinary tables only
          AND c.relnamespace = 'public'::regnamespace   -- 1C использует public schema
    ) classified
    GROUP BY category
    HAVING COUNT(*) > 0
) agg
...
-- ============================================================================
-- КОНЕЦ СКРИПТА СБОРА ДАННЫХ v4.0
-- ============================================================================
