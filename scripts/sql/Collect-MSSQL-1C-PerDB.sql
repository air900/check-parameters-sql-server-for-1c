...
SELECT 440 + ROW_NUMBER() OVER (ORDER BY DB_ID()) AS "N",
       ...
       CONVERT(NVARCHAR(20), COUNT(*)) + N' таблиц / макс '
         + CONVERT(NVARCHAR(20), ISNULL(MAX(sub.size_mb) / 1024, 0)) + N' GB' AS "Display",
       CONVERT(NVARCHAR(MAX),
         '{"db":"' + (DB_NAME() COLLATE DATABASE_DEFAULT) + '"'
         + ',"count":'      + CONVERT(NVARCHAR(20), COUNT(*))
         + ',"max_size_mb":'+ CONVERT(NVARCHAR(20), ISNULL(MAX(sub.size_mb), 0))
         + ',"sum_size_mb":'+ CONVERT(NVARCHAR(20), ISNULL(SUM(sub.size_mb), 0))
         + '}'
       ) AS "Value"
FROM (
    SELECT t.object_id,
           MAX(p.data_compression) AS max_compression,
    ...
    HAVING SUM(au.total_pages) * 8 / 1024 > 10240   -- > 10 GB
       AND MAX(p.data_compression) = 0              -- 0 = NONE (1=ROW, 2=PAGE, 3=COLUMNSTORE, 4=COLUMNSTORE_ARCHIVE)
) sub
WHERE DB_ID() > 4;
GO
