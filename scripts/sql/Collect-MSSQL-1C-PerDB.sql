...
;WITH categorized AS (
    SELECT t.object_id, t.name,
           CASE
                ...
                WHEN t.name LIKE '\_AccumReg%Totals' ESCAPE '\'                   THEN 'accumreg_totals'
                WHEN t.name LIKE '\_AccumReg%' ESCAPE '\'                         THEN 'accumreg_movements'
                WHEN t.name LIKE '\_InfoReg%' ESCAPE '\'                          THEN 'inforeg'
                WHEN t.name LIKE '\_AccReg%' ESCAPE '\'                           THEN 'accreg'
                WHEN t.name LIKE '\_CalcReg%' ESCAPE '\'                          THEN 'calcreg'
                WHEN t.name LIKE '\_Chrc%' ESCAPE '\'                             THEN 'chrc'
                WHEN t.name LIKE '\_AccChrt%' ESCAPE '\'                          THEN 'accchrt'
