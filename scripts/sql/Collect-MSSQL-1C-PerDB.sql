...
WHERE DB_ID() > 4
  ...
  AND sp.rows > 1000
  -- Реальный признак «нужно обновить»: либо много изменений (>20% строк),
  -- либо явно давно не трогали (>30 дней). Иначе попадание в top — шум.
  AND (sp.modification_counter > 0.2 * sp.rows
       OR DATEDIFF(DAY, sp.last_updated, GETDATE()) > 30)
-- Сортируем по % изменений сначала, потом по возрасту — самые «горячие»
-- кандидаты сверху, при равных нулях — самые старые крупные таблицы.
