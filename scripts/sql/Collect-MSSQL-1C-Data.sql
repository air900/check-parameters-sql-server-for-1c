...
SELECT 220 + ROW_NUMBER() OVER (ORDER BY name) AS "N",
       'recovery' AS "Section",
       '_db_' + (name COLLATE DATABASE_DEFAULT) AS "Key",
       N'БД: ' + (name COLLATE DATABASE_DEFAULT) AS "Label",
       'recovery=' + (recovery_model_desc COLLATE DATABASE_DEFAULT)
         + ' / shrink=' + CASE WHEN is_auto_shrink_on = 1 THEN 'ON' ELSE 'OFF' END
         + ' / collation=' + ISNULL(collation_name COLLATE DATABASE_DEFAULT, 'n/a')
         + ' / compat=' + CONVERT(NVARCHAR(10), compatibility_level)
         + ' / page_verify=' + (page_verify_option_desc COLLATE DATABASE_DEFAULT) AS "Display",
       CONVERT(NVARCHAR(MAX),
         '{"recovery":"' + (recovery_model_desc COLLATE DATABASE_DEFAULT) + '"'
         + ',"auto_shrink":' + CASE WHEN is_auto_shrink_on = 1 THEN 'true' ELSE 'false' END
         + ',"collation":"' + ISNULL(collation_name COLLATE DATABASE_DEFAULT, '') + '"'
         + ',"compat":' + CONVERT(NVARCHAR(10), compatibility_level)
         + ',"page_verify":"' + (page_verify_option_desc COLLATE DATABASE_DEFAULT) + '"'
         + ',"is_user_db":' + CASE WHEN database_id > 4 THEN 'true' ELSE 'false' END
         + '}'
       ) AS "Value"
FROM sys.databases
WHERE database_id <= 4 OR (state_desc COLLATE DATABASE_DEFAULT) = 'ONLINE';
GO
...
WHERE (sp.last_updated < DATEADD(DAY, -3, GETDATE())
  ...
  -- bd 70e: пропускаем системные базы (master/tempdb/model/msdb). Их статистика 1С не касается.
  AND DB_ID() > 4
