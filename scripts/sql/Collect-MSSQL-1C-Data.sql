...
UNION ALL
-- bd r2y: legacy_cardinality_estimator_db_count (требует функцию
-- sys.database_scoped_configurations, SQL 2016+) вынесен из CTE в version-gated batch.
...
-- ----------------------------------------------------------------------------
-- T-SQL static name resolution: column/function references bind at PARSE
-- time. Keys ниже используют столбцы (is_autogrow_all_files,
-- is_mixed_page_allocation_on) и функции (sys.database_scoped_configurations),
-- появившиеся в SQL 2016 RTM (ProductMajorVersion = 13). Если такой
-- запрос положить внутрь main CTE и запустить на SQL 2014 — весь CTE
-- упадёт с Msg 207 "Invalid column name" ДО возврата строк → пустой
-- diagnostic.
--
...
BEGIN
    ...
    EXEC sp_executesql N'
    SELECT 95 AS [N], ''instance_config'' AS [Section], ''legacy_cardinality_estimator_db_count'' AS [Key],
        N''Баз 1С с устаревшим оценщиком количества строк (LEGACY_CE = ON)'' AS [Label],
        CONVERT(NVARCHAR(20), COUNT(*)) AS [Display],
        CONVERT(NVARCHAR(20), COUNT(*)) AS [Value]
    FROM sys.databases d
    CROSS APPLY (
        SELECT [value]
        FROM sys.database_scoped_configurations(d.database_id)
        WHERE (name COLLATE DATABASE_DEFAULT) = ''LEGACY_CARDINALITY_ESTIMATION''
    ) dsc
    WHERE d.database_id > 4 AND CAST(dsc.[value] AS INT) = 1';
END
ELSE
BEGIN
    -- SQL 2014- placeholder: backend silently skips эти правила через
    -- float('n/a') ValueError. Это даёт корректный empty-state в HTML-отчёте
    -- ("параметр не проверялся — версия SQL ниже минимальной").
    SELECT 46 AS [N], 'tempdb' AS [Section], 'tempdb_autogrow_all_files' AS [Key],
        N'Все файлы tempdb растут одновременно (AUTOGROW_ALL_FILES)' AS [Label],
    ...
    UNION ALL
    SELECT 95, 'instance_config', 'legacy_cardinality_estimator_db_count',
        N'Баз 1С с устаревшим оценщиком количества строк (LEGACY_CE = ON)',
        N'не поддерживается (SQL < 2016)', 'n/a';
END
GO
