...
SELECT 114, 'maintenance',
    ...
    CASE WHEN
      ...
      OR EXISTS (
        ...
        WHERE j.enabled = 1
          AND ((sp.name COLLATE DATABASE_DEFAULT) LIKE '%Statist%'
            OR (sp.name COLLATE DATABASE_DEFAULT) LIKE '%статист%')
      )
...
SELECT 115, 'maintenance',
    ...
    CASE WHEN
      ...
      OR EXISTS (
        ...
        JOIN msdb.dbo.sysjobs j ON j.job_id = sp.job_id
        WHERE j.enabled = 1
          AND ((sp.name COLLATE DATABASE_DEFAULT) LIKE '%Index%'
            OR (sp.name COLLATE DATABASE_DEFAULT) LIKE '%индекс%'
            OR (sp.name COLLATE DATABASE_DEFAULT) LIKE '%Reorg%'
            OR (sp.name COLLATE DATABASE_DEFAULT) LIKE '%Rebuild%')
      )
...
SELECT 116, 'maintenance',
    ...
    CASE WHEN
      ...
      OR EXISTS (
        ...
        JOIN msdb.dbo.sysjobs j ON j.job_id = sp.job_id
        WHERE j.enabled = 1
          AND ((sp.name COLLATE DATABASE_DEFAULT) LIKE '%Integrity%'
            OR (sp.name COLLATE DATABASE_DEFAULT) LIKE '%целостн%'
            OR (sp.name COLLATE DATABASE_DEFAULT) LIKE '%CHECKDB%')
      )
      -- (D) PowerShell — dbatools
      OR EXISTS (
        SELECT 1 FROM msdb.dbo.sysjobsteps st
