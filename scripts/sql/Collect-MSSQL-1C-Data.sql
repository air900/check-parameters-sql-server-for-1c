...
UNION ALL
-- bd r2y: tempdb_autogrow_all_files (требует столбец is_autogrow_all_files,
-- SQL 2016+) вынесен из CTE в отдельный version-gated batch ниже.
...
UNION ALL
-- bd nbf: RCSI (READ_COMMITTED_SNAPSHOT) — критично для 1С 8.3, снимочное чтение
-- убирает ложные блокировки чтение↔запись на расчётах. Считаем сколько 1С-баз
-- запущено БЕЗ RCSI. Источник: gilev.ru, правило mssql_rcsi_disabled.
-- Столбец is_read_committed_snapshot_on есть в SQL 2005+, безопасно в CTE.
SELECT 56, 'recovery',
    'rcsi_disabled_db_count',
    N'Баз 1С без снимочного чтения (READ_COMMITTED_SNAPSHOT OFF)',
    CONVERT(NVARCHAR(20), COUNT(*)),
    CONVERT(NVARCHAR(20), COUNT(*))
FROM sys.databases
WHERE database_id > 4 AND is_read_committed_snapshot_on = 0

UNION ALL
-- bd r2y: mixed_page_allocation_user_db_count (требует столбец
-- is_mixed_page_allocation_on, SQL 2016+) вынесен из CTE в version-gated batch.
...
UNION ALL
-- bd fuo: legacy_cardinality_estimator_db_count перенесён в Collect-MSSQL-1C-PerDB.sql.
-- Причина: sys.database_scoped_configurations — это catalog view без параметров,
-- работающее ТОЛЬКО в контексте текущей БД. Раньше здесь был некорректный вызов
-- sys.database_scoped_configurations(d.database_id) как TVF (Msg 4121). Per-DB
-- коллектор уже исполняется через sqlcmd -d <db> в нужном контексте → запрос
-- работает естественно. Агрегат собирается в Invoke-MssqlDiagnostic.psm1.
...
SELECT 114, 'maintenance',
    ...
    N'Регламентное задание обновления статистики',
    CASE WHEN
      -- (A) TSQL subsystem — direct command match (legacy, was the only source pre-2.12.1)
      EXISTS (
        SELECT 1 FROM msdb.dbo.sysjobsteps st
        JOIN msdb.dbo.sysjobs j ON j.job_id = st.job_id
        WHERE j.enabled = 1
          AND st.subsystem = 'TSQL'
          AND (st.command LIKE '%UPDATE STATISTICS%'
            OR st.command LIKE '%sp_updatestats%'
            OR st.command LIKE '%@UpdateStatistics%'
            OR st.command LIKE '%IndexOptimize%')
      )
      -- (B) SSIS subsystem — Maintenance Plan tasks (was main false-negative pre-2.12.1)
      OR EXISTS (
        SELECT 1 FROM msdb.dbo.sysjobsteps st
        JOIN msdb.dbo.sysjobs j ON j.job_id = st.job_id
        WHERE j.enabled = 1
          AND st.subsystem = 'SSIS'
          AND (st.command LIKE '%Update Statistics Task%'
            OR st.command LIKE '%Обновление статистики%'
            OR st.command LIKE '%subplan%')
      )
      -- (C) sysmaintplan_subplans — direct Maintenance Plan structure
      OR EXISTS (
        SELECT 1 FROM msdb.dbo.sysmaintplan_subplans sp
        JOIN msdb.dbo.sysjobs j ON j.job_id = sp.job_id
        WHERE j.enabled = 1
          AND ((sp.subplan_name COLLATE DATABASE_DEFAULT) LIKE '%Statist%'
            OR (sp.subplan_name COLLATE DATABASE_DEFAULT) LIKE '%статист%')
      )
      -- (D) PowerShell subsystem — dbatools and custom cmdlets
      OR EXISTS (
        SELECT 1 FROM msdb.dbo.sysjobsteps st
        JOIN msdb.dbo.sysjobs j ON j.job_id = st.job_id
        WHERE j.enabled = 1
          AND st.subsystem = 'PowerShell'
          AND (st.command LIKE '%Invoke-DbaIndexOptimize%'
            OR st.command LIKE '%Update-DbaStat%'
            OR st.command LIKE '%sp_updatestats%')
      )
...
SELECT 115, 'maintenance',
    ...
    N'Регламентное задание обслуживания индексов',
    CASE WHEN
      -- (A) TSQL — direct ALTER INDEX or IndexOptimize
      EXISTS (
        SELECT 1 FROM msdb.dbo.sysjobsteps st
        JOIN msdb.dbo.sysjobs j ON j.job_id = st.job_id
        WHERE j.enabled = 1
          AND st.subsystem = 'TSQL'
          AND (st.command LIKE '%ALTER INDEX%REORGANIZE%'
            OR st.command LIKE '%ALTER INDEX%REBUILD%'
            OR st.command LIKE '%IndexOptimize%')
      )
      -- (B) SSIS — Maintenance Plan Reorganize/Rebuild Index Task
      OR EXISTS (
        SELECT 1 FROM msdb.dbo.sysjobsteps st
        JOIN msdb.dbo.sysjobs j ON j.job_id = st.job_id
        WHERE j.enabled = 1
          AND st.subsystem = 'SSIS'
          AND (st.command LIKE '%Reorganize Index Task%'
            OR st.command LIKE '%Rebuild Index Task%'
            OR st.command LIKE '%Реорганизация индексов%'
            OR st.command LIKE '%Перестройка индексов%'
            OR st.command LIKE '%subplan%')
      )
      -- (C) sysmaintplan_subplans direct
      OR EXISTS (
        SELECT 1 FROM msdb.dbo.sysmaintplan_subplans sp
        JOIN msdb.dbo.sysjobs j ON j.job_id = sp.job_id
        WHERE j.enabled = 1
          AND ((sp.subplan_name COLLATE DATABASE_DEFAULT) LIKE '%Index%'
            OR (sp.subplan_name COLLATE DATABASE_DEFAULT) LIKE '%индекс%'
            OR (sp.subplan_name COLLATE DATABASE_DEFAULT) LIKE '%Reorg%'
            OR (sp.subplan_name COLLATE DATABASE_DEFAULT) LIKE '%Rebuild%')
      )
      -- (D) PowerShell — dbatools
      OR EXISTS (
        SELECT 1 FROM msdb.dbo.sysjobsteps st
        JOIN msdb.dbo.sysjobs j ON j.job_id = st.job_id
        WHERE j.enabled = 1
          AND st.subsystem = 'PowerShell'
          AND (st.command LIKE '%Invoke-DbaIndexOptimize%'
            OR st.command LIKE '%Optimize-DbaIndex%')
      )
    THEN N'есть' ELSE N'нет' END,
    CASE WHEN
      EXISTS (
...
SELECT 116, 'maintenance',
    'checkdb_job_exists',
    N'Регламентное задание проверки целостности БД',
    CASE WHEN
      -- (A) TSQL — direct DBCC CHECKDB
      EXISTS (
        SELECT 1 FROM msdb.dbo.sysjobsteps st
        JOIN msdb.dbo.sysjobs j ON j.job_id = st.job_id
        WHERE j.enabled = 1
          AND st.subsystem = 'TSQL'
          AND (st.command LIKE '%DBCC CHECKDB%'
            OR st.command LIKE '%DatabaseIntegrityCheck%')
      )
      -- (B) SSIS — Maintenance Plan Check Database Integrity Task
      OR EXISTS (
        SELECT 1 FROM msdb.dbo.sysjobsteps st
        JOIN msdb.dbo.sysjobs j ON j.job_id = st.job_id
        WHERE j.enabled = 1
          AND st.subsystem = 'SSIS'
          AND (st.command LIKE '%Check Database Integrity Task%'
            OR st.command LIKE '%Проверка целостности баз данных%'
            OR st.command LIKE '%subplan%')
      )
      -- (C) sysmaintplan_subplans direct
      OR EXISTS (
        ...
        JOIN msdb.dbo.sysjobs j ON j.job_id = sp.job_id
        WHERE j.enabled = 1
          AND ((sp.subplan_name COLLATE DATABASE_DEFAULT) LIKE '%Integrity%'
            OR (sp.subplan_name COLLATE DATABASE_DEFAULT) LIKE '%целостн%'
            OR (sp.subplan_name COLLATE DATABASE_DEFAULT) LIKE '%CHECKDB%')
      )
      -- (D) PowerShell — dbatools
      OR EXISTS (
        SELECT 1 FROM msdb.dbo.sysjobsteps st
        ...
        WHERE j.enabled = 1
          AND st.subsystem = 'PowerShell'
          AND st.command LIKE '%Invoke-DbaDbCheck%'
      )
    THEN N'есть' ELSE N'нет' END,
    CASE WHEN
      EXISTS (
...
-- ============================================================================
-- bd r2y: SQL 2016+ specific keys (gated by ProductMajorVersion).
-- ----------------------------------------------------------------------------
-- T-SQL static name resolution: column references bind at PARSE time. Keys
-- ниже используют столбцы (is_autogrow_all_files, is_mixed_page_allocation_on),
-- появившиеся в SQL 2016 RTM (ProductMajorVersion = 13). Если такой запрос
-- положить внутрь main CTE и запустить на SQL 2014 — весь CTE упадёт с Msg 207
-- "Invalid column name" ДО возврата строк → пустой diagnostic.
--
-- Решение: вынести в отдельный batch ПОСЛЕ main GO. Batch гейтится через
-- IF SERVERPROPERTY. На SQL 2016+ — выполняем queries через sp_executesql
-- (bypass parse-time column resolution в outer batch). На SQL 2014- —
-- эмитим placeholder rows с Display='не поддерживается (SQL < 2016)' и
-- Value='n/a'. Backend evaluator: float('n/a') → ValueError → SKIPPED
-- (evaluators.py:290) → правила НЕ фаерят, но и не теряются в отчёте.
-- ============================================================================

IF CAST(SERVERPROPERTY('ProductMajorVersion') AS INT) >= 13
BEGIN
    EXEC sp_executesql N'
    SELECT 46 AS [N], ''tempdb'' AS [Section], ''tempdb_autogrow_all_files'' AS [Key],
        N''Все файлы tempdb растут одновременно (AUTOGROW_ALL_FILES)'' AS [Label],
        CASE WHEN MAX(CAST(is_autogrow_all_files AS INT)) = 1
             THEN N''включено'' ELSE N''выключено'' END AS [Display],
        CASE WHEN MAX(CAST(is_autogrow_all_files AS INT)) = 1
             THEN ''true'' ELSE ''false'' END AS [Value]
    FROM tempdb.sys.filegroups WHERE (name COLLATE DATABASE_DEFAULT) = ''PRIMARY''';

    EXEC sp_executesql N'
    SELECT 57 AS [N], ''recovery'' AS [Section], ''mixed_page_allocation_user_db_count'' AS [Key],
        N''Баз 1С со смешанным размещением страниц (MIXED_PAGE_ALLOCATION ON)'' AS [Label],
        CONVERT(NVARCHAR(20), COUNT(*)) AS [Display],
        CONVERT(NVARCHAR(20), COUNT(*)) AS [Value]
    FROM sys.databases
    WHERE database_id > 4 AND is_mixed_page_allocation_on = 1';
END
ELSE
BEGIN
    SELECT 46 AS [N], 'tempdb' AS [Section], 'tempdb_autogrow_all_files' AS [Key],
        N'Все файлы tempdb растут одновременно (AUTOGROW_ALL_FILES)' AS [Label],
        N'не поддерживается (SQL < 2016)' AS [Display],
        'n/a' AS [Value]
    UNION ALL
    SELECT 57, 'recovery', 'mixed_page_allocation_user_db_count',
        N'Баз 1С со смешанным размещением страниц (MIXED_PAGE_ALLOCATION ON)',
        N'не поддерживается (SQL < 2016)', 'n/a';
END
GO
...
-- ============================================================================
-- v2.12 — sys.configurations bulk-collect (replaces 8 per-name SELECT блоков).
-- ----------------------------------------------------------------------------
-- Backend (flatten_sys_configurations) разворачивает эту строку в те же flat-
-- ключи, что раньше эмитировал коллектор по одному на параметр (плюс ~96 опций
-- Tier-2, доступных forensic-only). FOR JSON PATH строит массив объектов из
-- sys.configurations. Присваиваем результат скаляру через sp_executesql, чтобы
-- обойти запрет FOR JSON в INSERT...EXEC (Msg 13602, SQL 2017+); здесь
-- подзапрос (SELECT … FOR JSON PATH) присваивается напрямую — это разрешено.
-- ============================================================================

DECLARE @sys_config_full NVARCHAR(MAX) = (
    SELECT
        name,
        value_in_use,
        [value],
...
SELECT 999 AS "N", 'instance_config' AS "Section",
    '_sys_configurations_full' AS "Key",
    N'Полный снимок sys.configurations (forensic, v2.12)' AS "Label",
    N'' AS "Display",
    ISNULL(@sys_config_full, N'[]') AS "Value";
GO
