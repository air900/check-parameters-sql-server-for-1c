...
function Show-AnalysisOffer {
    ...
    Write-Host ''
    Write-Host '  Углублённый анализ:' -ForegroundColor Cyan
    Write-Host '    - оценка текущих настроек сервера' -ForegroundColor White
    Write-Host '    - выявление ошибок конфигурации, влияющих на производительность 1С' -ForegroundColor White
    Write-Host '    - приоритезация: что критично, что желательно, что в норме' -ForegroundColor White
    Write-Host ''
}
...
.PARAMETER Dbms
    Имя СУБД — 'postgresql' или 'mssql'. Используется только при сборке fallback-payload
    (когда -Payload не передан). Если -Payload передан — поле берётся из него.
...
function Send-DiagnosticData {
    ...
    if (-not $Payload) {
        # Fallback-сборка payload из Results. dbms должен быть передан явно через -Dbms
        # (см. bd-ov1: ранее использовался $global:dbms / $script:dbms — переменная,
        # которая никогда не устанавливалась после переименования $dbms → $selectedEngine
        # в v2.7.2; .psm1 модуль имеет свой $script: scope и не видит переменные
        # вызывающего скрипта). Корректный путь — всегда передавать -Payload явно
        # (см. Invoke-1CDiagnostic.ps1); fallback оставлен на случай вызова модуля
        # из других контекстов.
        if (-not $Dbms) {
            throw "Send-DiagnosticData: при вызове без -Payload требуется -Dbms (postgresql|mssql)."
        }
        $Payload = @{
            version    = 'unknown'
            dbms       = $Dbms
