...
function Send-DiagnosticData {
    ...
    if (-not $Payload) {
        ...
        # ВНИМАНИЕ: при таком вызове dbms неизвестен — берём из глобальной переменной
        # $script:dbms / $global:dbms если задана, иначе 'postgresql' (исторический default).
        # Корректный путь — всегда передавать -Payload явно (см. Invoke-1CDiagnostic.ps1).
        $fallbackDbms = if ($global:dbms) { $global:dbms } elseif ($script:dbms) { $script:dbms } else { 'postgresql' }
        $Payload = @{
            version    = 'unknown'
            dbms       = $fallbackDbms
