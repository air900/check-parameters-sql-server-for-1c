...
function Show-Findings {
    param([array]$Findings, [hashtable]$BackendSummary)
...
function Send-DiagnosticData {
    ...
    try {
        ...
        # Сообщаем report_id для возможной загрузки HTML-отчёта
        if ($response.report_url) {
            Write-Host ''
            Write-Host "  Полный отчёт доступен по ссылке:" -ForegroundColor Gray
            Write-Host "  $($response.report_url)" -ForegroundColor Cyan
        }
    }
    catch {
        Write-Host ''
