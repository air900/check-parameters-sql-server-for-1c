...
function Get-MssqlOSContext {
    ...
    .DESCRIPTION
        ...
        Структура вывода:
        - N=700-706: АГРЕГАТНЫЕ ключи (для оценки правилами с min/max/boolean эвалюаторами)
        ...
        себя как failed при ошибке (нет admin, WMI заблокирован GPO, нет Defender в RU
        окружениях с Kaspersky/ESET, powercfg недоступен). На failure-пути функция всё
        равно эмитит строку с Display='не удалось определить — недостаточно прав / WMI
        заблокирован / нет компонента' и Value='unknown' — чтобы YAML-правило
        mssql_os_context_collection_failed создало INFO-finding и отчёт не был
        silent-clean. Аггрегат os_context_collection_failures (N=706) показывает
        общее число неудачных sub-проверок (0..5).
    ...
    # Учёт неудач по каждой sub-проверке (5 sub-проверок).
    # Каждая фиксируется в $subFailed[<name>] = $true при сбое — затем используется
    # для (а) выбора Display на failure-пути, (б) аггрегата os_context_collection_failures.
    $subFailed = @{
        'process_detection' = $false   # Get-Process 1C-процессов
        'win32_volume'      = $false   # Get-WmiObject Win32_Volume (block size)
        'win32_logicaldisk' = $false   # Get-WmiObject Win32_LogicalDisk (compression)
        'defender'          = $false   # Get-MpPreference (AV exclusions)
        'powercfg'          = $false   # powercfg /getactivescheme
    }
    ...
    try {
        ...
        $subFailed['process_detection'] = $true
    }

    if ($subFailed['process_detection']) {
    ...
    foreach ($drive in $dataDrives) {
        ...
        try {
            ...
            $subFailed['win32_logicaldisk'] = $true
        }
    }
    ...
    if ($dataDrives.Count -gt 0 -and -not $volQuerySucceeded) { $subFailed['win32_volume']      = $true }
    if ($dataDrives.Count -gt 0 -and -not $ldQuerySucceeded)  { $subFailed['win32_logicaldisk'] = $true }

    if ($subFailed['win32_volume']) {
        $rows.Add((New-MssqlOsRow `
    ...
    if ($subFailed['win32_logicaldisk']) {
        $rows.Add((New-MssqlOsRow `
    ...
    # Sub-проверка считается failed, если Get-MpPreference кинул исключение или
    # вернул $null (типично: Defender отключён / отсутствует — Kaspersky, ESET в RU).
    ...
    try {
        ...
        if ($mp) {
            ...
            $subFailed['defender'] = $true
        }
    } catch {
        $subFailed['defender'] = $true
    }

    if ($subFailed['defender']) {
        $rows.Add((New-MssqlOsRow `
    ...
    # Sub-проверка считается failed, если powercfg недоступен или не распарсился вывод.
    $highPerfGuids = @(
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0 -or -not $output) {
            $subFailed['powercfg'] = $true
        }
        elseif ($output -match 'GUID:\s*([0-9a-fA-F-]+)') {
            $activeGuid = $Matches[1].ToLower()
            $isHighPerf = ($highPerfGuids -contains $activeGuid)
        }
        else {
            # powercfg отработал, но GUID не нашли в выводе — считаем failure парсинга.
            $subFailed['powercfg'] = $true
        }
    } catch {
        $subFailed['powercfg'] = $true
    ...
    # 6. Аггрегат: число неудачных sub-проверок (для правила mssql_os_context_collection_failed) — N=706
    # Эмитим всегда — даже когда все sub-проверки успешны (значение 0). Это позволяет
    # backend-правилу различать «контекст собран чисто» vs «контекст частично/полностью не собран».
    $failedCount = (@($subFailed.Values) | Where-Object { $_ -eq $true }).Count
    $totalSub    = $subFailed.Count
    $failedNames = @($subFailed.GetEnumerator() | Where-Object { $_.Value -eq $true } | ForEach-Object { $_.Key })
    if ($failedCount -eq 0) {
        $failDisplay = "0 из $totalSub — все OS-проверки успешно выполнены"
    } else {
        $failDisplay = "$failedCount из $totalSub — не выполнены: $([string]::Join(', ', $failedNames))"
    }
    $rows.Add((New-MssqlOsRow `
