...
function Invoke-SqlcmdRaw {
    ...
    # -f 65001: force UTF-8 для INPUT (наши .sql c BOM). На русском Windows
    #           sqlcmd иногда не распознаёт UTF-8 BOM сам и читает файл как
    #           ANSI/cp1251. Тогда даже ASCII-идентификаторы (name, value)
    #           ломаются на сервере: "Недопустимое имя столбца 'пате'".
    #           См. bd-wox.
