...
function Invoke-MssqlSqlDiagnostic {
    ...
    $totalLegacyCeOnCount  = 0     # bd fuo — сумма (не max): сколько всего БД с LEGACY_CE = ON
    foreach ($r in $parsed) {
        ...
        try {
            ...
            elseif ($r.Key -like '_legacy_ce_*') {
                # bd fuo: SUM, а не MAX — каждая per-DB строка несёт 0 или 1
                # (LEGACY_CE on/off в этой БД), нужен общий счётчик.
                $j = ConvertFrom-Json $r.Value -ErrorAction Stop
                if ($null -ne $j.count) { $totalLegacyCeOnCount += [int]$j.count }
            }
        } catch {
    ...
    Write-Verbose "Server-wide rollup: high_change=$worstHighChange, max_age=$worstMaxAge, older_7d=$worstOlder7d, apl_pct=$worstAplPct, legacy_ce=$totalLegacyCeOnCount"
