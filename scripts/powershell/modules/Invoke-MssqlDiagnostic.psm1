...
function Invoke-SqlcmdRaw {
    ...
    #
    # Кодировку INPUT-файла НЕ задаём через -f — на русском Windows у sqlcmd
    # 13/14/15 BOM-detect для UTF-8 нестабилен (UTF-8-BOM .sql превращался
    # в кашу: ASCII «name» рендерилось серверу как кириллица «пате», а попытка
    # явно поставить -f 65001 ломала stderr-вывод). См. bd-wox.
    # Вместо этого ниже перепаковываем InputFile в UTF-16 LE с BOM во
    # временный файл — UTF-16 BOM sqlcmd распознаёт ОДНОЗНАЧНО на всех
    # версиях/локалях.
    $allArgs += '-u'
    $allArgs += '-b'
    $allArgs += '-r'; $allArgs += '1'
    ...
    # Перепаковка InputFile в UTF-16 LE BOM (см. комментарий выше). Tempfile
    # удаляется в finally вместе с tmpOut.
    $tmpIn = $null
    if (-not [string]::IsNullOrEmpty($InputFile)) {
        $tmpIn = [System.IO.Path]::GetTempFileName()
        $sqlBytes = [System.IO.File]::ReadAllBytes($InputFile)
        # Снимаем UTF-8 BOM (0xEF 0xBB 0xBF) если есть, остальное декодируем
        # как UTF-8 (наши .sql всегда сохраняем UTF-8 BOM).
        $offset = 0
        if ($sqlBytes.Length -ge 3 -and $sqlBytes[0] -eq 0xEF -and $sqlBytes[1] -eq 0xBB -and $sqlBytes[2] -eq 0xBF) {
            $offset = 3
        }
        $sqlText = [System.Text.Encoding]::UTF8.GetString($sqlBytes, $offset, $sqlBytes.Length - $offset)
        # Запись с UTF-16 LE BOM (Encoding.Unicode по умолчанию пишет с BOM).
        [System.IO.File]::WriteAllText($tmpIn, $sqlText, [System.Text.Encoding]::Unicode)
        $allArgs += '-i'; $allArgs += $tmpIn
    }
    ...
    try {
        ...
        if ($tmpIn) { Remove-Item -LiteralPath $tmpIn -ErrorAction SilentlyContinue }
    }
...
function Invoke-MssqlSqlDiagnostic {
    ...
    if ($null -ne $perDbScriptPath -and (Test-Path -LiteralPath $perDbScriptPath -PathType Leaf)) {
        ...
        # bd 37r-scope: per-DB итерация — ТОЛЬКО на выбранную $Database (если это
        # пользовательская БД, не master/tempdb/model/msdb). Customer указывает
        # конкретную БД для диагностики — нет смысла сканировать соседние 1С-базы
        # на сервере (это PostgreSQL-подход и так работает: один -d <db>).
        # Если -Database = master (default) → сканируем все 1С-БД (legacy fallback
        # для use-case "просто запусти на сервере, найди что есть").
        $systemDbs = @('master', 'tempdb', 'model', 'msdb')
        $oneCDatabases = @()

        if ($systemDbs -notcontains $Database.ToLower()) {
            # Указана конкретная пользовательская БД — итерируем только её.
            $oneCDatabases = @($Database)
            Write-Verbose "Per-DB iteration: scope=selected → только '$Database' (как PostgreSQL-подход)"
        }
        else {
            # -Database = master (default) → старое поведение: сканируем все 1С-базы.
            try {
            ...
            Write-Verbose "Per-DB iteration: scope=discovery (master mode) → найдено 1С-баз: $($oneCDatabases.Count)"
        }

        if ($oneCDatabases.Count -gt 0) {
            # bd 37r-warn: per-DB скрипт включает sys.dm_db_index_physical_stats(LIMITED)
            # — на крупной 1С-БД (>100 GB, 40k+ индексов) эта DMV занимает 2-7 минут.
            # Пользователю важно понять что не повисло — выводим явное предупреждение.
            $scopeLabel = if ($oneCDatabases.Count -eq 1) {
                "Анализ статистики и фрагментации индексов БД '$($oneCDatabases[0])'..."
            } else {
                "Анализ статистики и фрагментации индексов на каждой 1С-базе ($($oneCDatabases.Count) шт.)..."
            }
            Write-Host ''
            Write-Host "  $scopeLabel" -ForegroundColor Yellow
            Write-Host '  Операция может занять 2-7 минут на крупной БД (>100 GB, 40k+ индексов).' -ForegroundColor Yellow
            Write-Host '  Не прерывайте — это самая длительная фаза диагностики.' -ForegroundColor Yellow
            Write-Host ''
            ...
            foreach ($dbName in $oneCDatabases) {
                ...
                $dbIdx++
                $dbStart = Get-Date
                Write-Host ('  [{0}/{1}] {2} — анализ...' -f $dbIdx, $oneCDatabases.Count, $dbName) -ForegroundColor Gray -NoNewline
                try {
                    ...
                    if ($perDbResult.ExitCode -ne 0) {
                        ...
                        Write-Host '' # break the inline progress line before warning
                    ...
                    # Завершить inline-прогресс «X — анализ...» с длительностью
                    $dbSecs = [int][math]::Round((New-TimeSpan -Start $dbStart -End (Get-Date)).TotalSeconds)
                    Write-Host (' готово за {0} сек' -f $dbSecs) -ForegroundColor Green
                }
                catch {
                    Write-Host '' # break the inline progress line before warning
    ...
    # ------------------------------------------------------------------
    # Server-wide rollup из per-DB summary-rows (bd y49).
    # Скан _stats_summary_<db> и _apl_<db> по всем БД → агрегаты «worst-case
    # по всем БД». Нужно для YAML-правил уровня сервера (per-DB rules не
    # поддерживаются rule_engine'ом, который работает по exact param key).
    # Эмитим, даже если ни один _stats_summary не нашёлся (значение 0) —
    # отсутствие aggregate-row в отчёте интерпретируется как «данных нет».
    # ------------------------------------------------------------------
    $worstHighChange       = 0
    $worstMaxAge           = 0
    $worstOlder7d          = 0
    $worstOlder30d         = 0
    $worstAplPct           = 0.0
    $worstIndexHighFrag    = 0     # bd 37r
    $worstIndexMaxFragPct  = 0.0   # bd 37r
    $worstLargeUncompCount = 0     # bd nbf
    # bd 5ni v1.7 — counts from per-DB JSON aggregation (sums, not max)
    $totalLegacyCeOnCount       = 0   # _dsc_legacy_cardinality_estimation_*  (v1.6 → v1.7-B unified)
    $totalDscMaxdopOverride     = 0   # _dsc_maxdop_* where value != 0 (default)
    $totalDscParamSniffingOff   = 0   # _dsc_parameter_sniffing_* where value = 0
    $totalQsOffCount            = 0   # _query_store_* where actual_state = 0
    $totalQsReadOnlyCount       = 0   # _query_store_* where actual_state = 1
    $totalTrustworthyOnCount    = 0   # _trustworthy_* where trustworthy_on = 1
    $totalAdrDisabledLargeCount = 0   # _adr_* where adr_on = 0 AND size_gb >= 100
    foreach ($r in $parsed) {
        if ($null -eq $r -or [string]::IsNullOrEmpty($r.Key) -or [string]::IsNullOrEmpty($r.Value)) { continue }
        try {
            if ($r.Key -like '_stats_summary_*') {
                $j = ConvertFrom-Json $r.Value -ErrorAction Stop
                if ($null -ne $j.high_change   -and [int]$j.high_change   -gt $worstHighChange) { $worstHighChange = [int]$j.high_change }
                if ($null -ne $j.max_age_days  -and [int]$j.max_age_days  -gt $worstMaxAge)    { $worstMaxAge     = [int]$j.max_age_days }
                if ($null -ne $j.older_7d      -and [int]$j.older_7d      -gt $worstOlder7d)   { $worstOlder7d    = [int]$j.older_7d }
                if ($null -ne $j.older_30d     -and [int]$j.older_30d     -gt $worstOlder30d)  { $worstOlder30d   = [int]$j.older_30d }
            }
            elseif ($r.Key -like '_apl_*') {
                $j = ConvertFrom-Json $r.Value -ErrorAction Stop
                if ($null -ne $j.pct -and [double]$j.pct -gt $worstAplPct) { $worstAplPct = [double]$j.pct }
            }
            elseif ($r.Key -like '_index_fragmentation_summary_*') {
                $j = ConvertFrom-Json $r.Value -ErrorAction Stop
                if ($null -ne $j.high_frag_count -and [int]$j.high_frag_count -gt $worstIndexHighFrag)   { $worstIndexHighFrag   = [int]$j.high_frag_count }
                if ($null -ne $j.max_frag_pct    -and [double]$j.max_frag_pct -gt $worstIndexMaxFragPct) { $worstIndexMaxFragPct = [double]$j.max_frag_pct }
            }
            elseif ($r.Key -like '_large_uncompressed_*') {
                $j = ConvertFrom-Json $r.Value -ErrorAction Stop
                if ($null -ne $j.count -and [int]$j.count -gt $worstLargeUncompCount) { $worstLargeUncompCount = [int]$j.count }
            }
            elseif ($r.Key -like '_dsc_*') {
                # bd 5ni v1.7-B: unified scoped-config dispatcher.
                # Каждая per-DB строка = одна опция в одной БД, поле name в JSON
                # говорит какая. Раскладываем по соответствующим бакетам.
                $j = ConvertFrom-Json $r.Value -ErrorAction Stop
                switch ($j.name) {
                    'LEGACY_CARDINALITY_ESTIMATION' {
                        if ([int]$j.value -eq 1) { $totalLegacyCeOnCount++ }
                    }
                    'MAXDOP' {
                        # Override считается, если value != 0 (0 = "use server default").
                        if ([int]$j.value -ne 0) { $totalDscMaxdopOverride++ }
                    }
                    'PARAMETER_SNIFFING' {
                        if ([int]$j.value -eq 0) { $totalDscParamSniffingOff++ }
                    }
                }
            }
            elseif ($r.Key -like '_query_store_*') {
                # bd 5ni v1.7-A
                $j = ConvertFrom-Json $r.Value -ErrorAction Stop
                if ($null -ne $j.actual_state) {
                    if ([int]$j.actual_state -eq 0) { $totalQsOffCount++ }
                    elseif ([int]$j.actual_state -eq 1) { $totalQsReadOnlyCount++ }
                }
            }
            elseif ($r.Key -like '_trustworthy_*') {
                # bd 5ni v1.7-C
                $j = ConvertFrom-Json $r.Value -ErrorAction Stop
                if ($null -ne $j.trustworthy_on -and [int]$j.trustworthy_on -eq 1) {
                    $totalTrustworthyOnCount++
                }
            }
            elseif ($r.Key -like '_adr_*') {
                # bd 5ni v1.7-D — ADR off + БД крупная (>= 100 GB)
                $j = ConvertFrom-Json $r.Value -ErrorAction Stop
                if ($null -ne $j.adr_on -and [int]$j.adr_on -eq 0 -and $null -ne $j.size_gb -and [int]$j.size_gb -ge 100) {
                    $totalAdrDisabledLargeCount++
                }
            }
        } catch {
            # Молча пропускаем строки с битым/отсутствующим JSON Value —
            # это диагностический агрегат, лучше показать 0 чем сломать pipeline.
        }
    }

    # bd ar9 follow-up: Value ВСЕГДА в InvariantCulture (точка-разделитель),
    # иначе на RU-локали '99,600' backend парсит как 99600 → правила
    # threshold-based silent-fail. Display может быть локализован.
    $inv = [System.Globalization.CultureInfo]::InvariantCulture
    $aggRows = @(
        [PSCustomObject]@{ N = 750; Section = 'statistics'; Key = 'worst_stats_high_change_count';
            Label = 'Макс. количество статистик с >20% изменений (по всем БД)';
            Display = "$worstHighChange"; Value = "$worstHighChange" },
        [PSCustomObject]@{ N = 751; Section = 'statistics'; Key = 'worst_stats_max_age_days';
            Label = 'Макс. возраст статистик (по всем БД), дн';
            Display = "$worstMaxAge"; Value = "$worstMaxAge" },
        [PSCustomObject]@{ N = 752; Section = 'statistics'; Key = 'worst_stats_older_7d_count';
            Label = 'Макс. количество статистик старше 7 дней (по всем БД)';
            Display = "$worstOlder7d"; Value = "$worstOlder7d" },
        [PSCustomObject]@{ N = 753; Section = 'statistics'; Key = 'worst_stats_older_30d_count';
            Label = 'Макс. количество статистик старше 30 дней (по всем БД)';
            Display = "$worstOlder30d"; Value = "$worstOlder30d" },
        [PSCustomObject]@{ N = 754; Section = 'statistics'; Key = 'worst_allow_page_locks_off_pct';
            Label = 'Макс. % индексов без ALLOW_PAGE_LOCKS=ON (по всем БД)';
            Display = ([string]::Format($inv, '{0:F1} %', $worstAplPct));
            Value   = ([string]::Format($inv, '{0:F3}',  $worstAplPct)) },
        # bd 37r — фрагментация индексов
        [PSCustomObject]@{ N = 755; Section = 'maintenance'; Key = 'worst_index_high_frag_count';
            Label = 'Макс. количество индексов с фрагментацией > 30% (по всем БД)';
            Display = "$worstIndexHighFrag"; Value = "$worstIndexHighFrag" },
        [PSCustomObject]@{ N = 756; Section = 'maintenance'; Key = 'worst_index_max_frag_pct';
            Label = 'Макс. % фрагментации индекса (по всем БД)';
            Display = ([string]::Format($inv, '{0:F1} %', $worstIndexMaxFragPct));
            Value   = ([string]::Format($inv, '{0:F1}',   $worstIndexMaxFragPct)) },
        # bd nbf — крупные несжатые таблицы (> 10 GB)
        [PSCustomObject]@{ N = 757; Section = 'maintenance'; Key = 'worst_large_uncompressed_table_count';
            Label = 'Макс. количество несжатых таблиц > 10 ГБ (по всем БД)';
            Display = "$worstLargeUncompCount"; Value = "$worstLargeUncompCount" },
        # bd fuo — сколько 1С-БД с LEGACY_CARDINALITY_ESTIMATION = ON
        # Имя ключа совпадает с YAML param mssql_legacy_cardinality_estimator_on,
        # поэтому правило подхватывает rollup без правок.
        [PSCustomObject]@{ N = 758; Section = 'instance_config'; Key = 'legacy_cardinality_estimator_db_count';
            Label = 'Баз 1С с устаревшим оценщиком количества строк (LEGACY_CE = ON)';
            Display = "$totalLegacyCeOnCount"; Value = "$totalLegacyCeOnCount" },
        # bd 5ni v1.7-B — DB-scoped MAXDOP override
        [PSCustomObject]@{ N = 760; Section = 'instance_config'; Key = 'dsc_maxdop_override_db_count';
            Label = 'Баз 1С с переопределением MAXDOP в БД (override server)';
            Display = "$totalDscMaxdopOverride"; Value = "$totalDscMaxdopOverride" },
        # bd 5ni v1.7-B — PARAMETER_SNIFFING = OFF
        [PSCustomObject]@{ N = 761; Section = 'instance_config'; Key = 'dsc_parameter_sniffing_off_db_count';
            Label = 'Баз 1С с выключенным parameter sniffing';
            Display = "$totalDscParamSniffingOff"; Value = "$totalDscParamSniffingOff" },
        # bd 5ni v1.7-A — Query Store OFF
        [PSCustomObject]@{ N = 762; Section = 'query_store'; Key = 'query_store_off_db_count';
            Label = 'Баз 1С с выключенным Query Store';
            Display = "$totalQsOffCount"; Value = "$totalQsOffCount" },
        # bd 5ni v1.7-A — Query Store READ_ONLY (overflowed)
        [PSCustomObject]@{ N = 763; Section = 'query_store'; Key = 'query_store_read_only_db_count';
            Label = 'Баз 1С с переполненным Query Store (READ_ONLY)';
            Display = "$totalQsReadOnlyCount"; Value = "$totalQsReadOnlyCount" },
        # bd 5ni v1.7-C — TRUSTWORTHY ON (security)
        [PSCustomObject]@{ N = 764; Section = 'security'; Key = 'trustworthy_on_db_count';
            Label = 'Баз 1С с TRUSTWORTHY = ON (security risk)';
            Display = "$totalTrustworthyOnCount"; Value = "$totalTrustworthyOnCount" },
        # bd 5ni v1.7-D — ADR disabled on large DBs
        [PSCustomObject]@{ N = 765; Section = 'recovery'; Key = 'adr_disabled_large_db_count';
            Label = 'Крупных 1С-баз (>=100 ГБ) без ускоренного восстановления';
            Display = "$totalAdrDisabledLargeCount"; Value = "$totalAdrDisabledLargeCount" }
    )
    $parsed = @($parsed) + $aggRows
    Write-Verbose "Server-wide rollup: high_change=$worstHighChange, max_age=$worstMaxAge, older_7d=$worstOlder7d, apl_pct=$worstAplPct, legacy_ce=$totalLegacyCeOnCount, dsc_maxdop=$totalDscMaxdopOverride, dsc_parm=$totalDscParamSniffingOff, qs_off=$totalQsOffCount, qs_ro=$totalQsReadOnlyCount, trustworthy=$totalTrustworthyOnCount, adr_off_large=$totalAdrDisabledLargeCount"
