...
function Invoke-MssqlSqlDiagnostic {
    ...
    if ($null -ne $perDbScriptPath -and (Test-Path -LiteralPath $perDbScriptPath -PathType Leaf)) {
        ...
        if ($oneCDatabases.Count -gt 0) {
            ...
            Write-Host ''
            Write-Host '  Анализ статистики и фрагментации индексов на каждой 1С-базе...' -ForegroundColor Yellow
            Write-Host ('  Найдено баз: {0}. Операция может занять 2-7 минут на одну крупную БД (>100 GB).' -f $oneCDatabases.Count) -ForegroundColor Yellow
