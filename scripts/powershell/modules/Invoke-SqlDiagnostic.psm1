...
function Find-1CDatabases {
    ...
    # statement_timeout через PGOPTIONS — не нужен SET в строке запроса
    # 120 сек — достаточно даже для серверов с десятками крупных баз
    ...
    # превращается в исключение и fallback-логика не срабатывает (PS 5.1)
    ...
    # Если запрос размеров не удался — fallback: список баз без размеров
    # ВАЖНО: fallback-запрос только ASCII — кириллица в аргументах cmd ломает кодировку на Windows
    if ($exitCode -ne 0) {
        Write-Warning "Query with sizes failed. Requesting list without sizes..."
        $env:PGOPTIONS = ''
        $fallbackQuery = "SELECT datname || '|' || '(n/a)' FROM pg_database WHERE datistemplate = false AND datname <> 'postgres' ORDER BY datname;"
