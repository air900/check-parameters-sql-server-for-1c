...
function Find-1CDatabases {
    ...
    # Subquery: pg_database_size() вызывается один раз на строку (а не дважды — в SELECT и ORDER BY)
    # statement_timeout = 30 сек — на серверах с десятками крупных баз pg_database_size() может занять минуты
    $listDbQuery = "SET statement_timeout = '30s'; SELECT datname || '|' || pg_size_pretty(sz) FROM (SELECT datname, pg_database_size(datname) AS sz FROM pg_database WHERE datistemplate = false AND datname <> 'postgres') t ORDER BY sz DESC;"
    $psqlArgs = @(
        '--host', $PgHost,
        '--port', $Port,
        '--username', $Username,
    ...
    # Временно отключаем $ErrorActionPreference = "Stop" — иначе stderr от psql
    # превращается в исключение и fallback-логика не срабатывает
    ...
    # Если запрос размеров превысил таймаут — fallback: список баз без размеров
    if ($exitCode -ne 0) {
        Write-Warning "Не удалось получить размеры баз данных. Запрашиваем список без размеров..."
        $fallbackQuery = "SELECT datname || '|' || '(размер недоступен)' FROM pg_database WHERE datistemplate = false AND datname <> 'postgres' ORDER BY datname;"
        $dbListOutput = & $PsqlPath '--host' $PgHost '--port' $Port '--username' $Username '--tuples-only' '--no-align' '--command' $fallbackQuery 'postgres' 2>&1
        $exitCode = $LASTEXITCODE
