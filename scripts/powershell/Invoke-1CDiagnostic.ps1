...
if (-not $Database) {
    ...
    if ($psqlForSearch) {
        Write-Host "  Запрос списка баз данных..." -ForegroundColor Gray

        # Запускаем поиск баз с индикацией прогресса (pg_database_size может быть медленным)
        # Start-Job в PS 5.1 НЕ наследует $env: — передаём PGPASSWORD явно через аргумент
        $pgPass = $env:PGPASSWORD
        $dbSearchJob = Start-Job -ScriptBlock {
            param($modulePath, $psqlPath, $pgHost, $port, $username, $pass)
            $env:PGPASSWORD = $pass
            Import-Module $modulePath -Force -DisableNameChecking
            Find-1CDatabases -PsqlPath $psqlPath -PgHost $pgHost -Port $port -Username $username
        } -ArgumentList (Join-Path $modulesPath 'Invoke-SqlDiagnostic.psm1'), $psqlForSearch, $PgHost, $Port, $Username, $pgPass

        # Индикатор активности: точки каждые 2 секунды + предупреждение через 10 сек
        $elapsed = 0
        $warned = $false
        while ($dbSearchJob.State -eq 'Running') {
            Start-Sleep -Seconds 2
            $elapsed += 2
            Write-Host "." -NoNewline -ForegroundColor Gray
            if ($elapsed -ge 10 -and -not $warned) {
                Write-Host ""
                Write-Host "  Получение размеров баз занимает время на серверах с большим количеством БД..." -ForegroundColor Yellow
                $warned = $true
            }
        }
        Write-Host ""  # перенос строки после точек

        # Фильтруем WarningRecord — оставляем только PSCustomObject с полем Name
        $databases = @(Receive-Job -Job $dbSearchJob -ErrorAction SilentlyContinue |
            Where-Object { $_ -is [PSCustomObject] -and $_.Name })
        Remove-Job -Job $dbSearchJob -Force -ErrorAction SilentlyContinue
