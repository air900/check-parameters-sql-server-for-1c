...
Write-Host ""
Write-Host "  $($project.display_name)  v$($project.version)  ($($project.vendor))" -ForegroundColor Cyan
Write-Host ""
...
elseif ($DBMS -eq 'mssql') {
    ...
    elseif ($pgFound -and $mssqlFound) {
        ...
        elseif ($mssqlRunning -and -not $pgRunning) {
            ...
            if ($choice.Trim() -eq '2') { $selectedEngine = 'mssql' }
            else                         { $selectedEngine = 'postgresql' }
        }
    }
    else {
        Write-Host ""
...
if ($selectedEngine -eq 'postgresql') {
...
if ($pgInstances.Count -gt 1) {
    Write-Host "  Найдено экземпляров: $($pgInstances.Count). Используется первый из них." -ForegroundColor DarkYellow
    ...
    if ($psqlForSearch) {
        ...
        elseif ($databases.Count -eq 1) {
            ...
            if ([int]::TryParse($choice, [ref]$choiceIdx) -and $choiceIdx -ge 1 -and $choiceIdx -le $databases.Count) {
                ...
                $Database = $databases[0].Name
                Write-Host "  Некорректный выбор, используется: $Database" -ForegroundColor Yellow
            }
...
elseif ($selectedEngine -eq 'mssql') {
...
if ($mssqlInstances.Count -gt 1) {
    Write-Host "  Найдено экземпляров: $($mssqlInstances.Count). Используется '$($ms.InstanceName)'." -ForegroundColor DarkYellow
...
if ([string]::IsNullOrEmpty($Database)) {
    ...
    if ($oneCdbs.Count -eq 0) {
        Write-Host "  Базы 1С не обнаружены, используется master." -ForegroundColor Yellow
        $Database = 'master'
    }
...
# ВАЖНО: pg_version в backend ServerInfo — required string ("" по умолчанию).
# null здесь даст HTTP 422 при отправке. Передаём пустую строку для совместимости
# (а MSSQL-специфичные поля mssql_version/mssql_edition/mssql_instance проходят
# через extra-allow в pydantic-схеме).
$serverInfo = @{
    pg_version    = ""
...
if (-not $NoPrompt) {
    ...
    Send-DiagnosticData -Results $results -Summary $summary -Payload $payload -Dbms $selectedEngine -ApiUrl $analyzeUrl
}
