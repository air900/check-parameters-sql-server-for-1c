...
if (-not $Database) {
    ...
    if ($psqlForSearch) {
        ...
        # Start-Job наследует $env:PGPASSWORD от родительского процесса
        $dbSearchJob = Start-Job -ScriptBlock {
            param($modulePath, $psqlPath, $pgHost, $port, $username)
            Import-Module $modulePath -Force -DisableNameChecking
            Find-1CDatabases -PsqlPath $psqlPath -PgHost $pgHost -Port $port -Username $username
        } -ArgumentList (Join-Path $modulesPath 'Invoke-SqlDiagnostic.psm1'), $psqlForSearch, $PgHost, $Port, $Username
