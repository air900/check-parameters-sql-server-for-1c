...
DECLARE @requests NVARCHAR(MAX), @waits NVARCHAR(MAX), @locks NVARCHAR(MAX),
        ...
        @deadlocks NVARCHAR(MAX), @plan_cache_top_cpu NVARCHAR(MAX),
        @plan_cache_top_reads NVARCHAR(MAX), @plan_variance NVARCHAR(MAX);
...
-- 8. deadlocks (system_health ring buffer; raw XML out, parse on backend)
SET @deadlocks = (
    SELECT TOP 50
        x.event_xml.value('(@timestamp)[1]', 'NVARCHAR(50)') AS event_timestamp_utc,
        CONVERT(NVARCHAR(MAX), x.event_xml.query('.')) AS event_xml_raw
    FROM (
        SELECT CONVERT(XML, st.target_data) AS target_xml
        FROM sys.dm_xe_session_targets st
        JOIN sys.dm_xe_sessions s ON s.address = st.event_session_address
        WHERE s.name = N'system_health' AND st.target_name = N'ring_buffer'
    ) AS rb
    CROSS APPLY rb.target_xml.nodes('//RingBufferTarget/event[@name="xml_deadlock_report"]') AS x(event_xml)
    ORDER BY 1 DESC
    FOR JSON PATH
);

-- 9. plan_cache_top_cpu (TOP 10 by total_worker_time)
SET @plan_cache_top_cpu = (
    SELECT TOP 10
        qs.execution_count, qs.total_worker_time, qs.total_logical_reads,
        qs.total_logical_writes, qs.total_physical_reads,
        qs.total_elapsed_time, qs.total_grant_kb, qs.last_execution_time,
        LEFT(SUBSTRING(st.text, qs.statement_start_offset/2 + 1,
            (CASE qs.statement_end_offset WHEN -1 THEN DATALENGTH(st.text)
             ELSE qs.statement_end_offset END - qs.statement_start_offset)/2 + 1), 16384) AS sql_text_statement,
        CONVERT(VARCHAR(34), qs.plan_handle, 1) AS plan_handle_hex,
        CONVERT(VARCHAR(34), qs.sql_handle, 1) AS sql_handle_hex,
        CONVERT(NVARCHAR(MAX), qp.query_plan) AS query_plan
...
-- 10. plan_cache_top_reads (TOP 10 by total_logical_reads)
SET @plan_cache_top_reads = (
    SELECT TOP 10
        qs.execution_count, qs.total_worker_time, qs.total_logical_reads,
        qs.total_logical_writes, qs.total_physical_reads,
        qs.total_elapsed_time, qs.total_grant_kb, qs.last_execution_time,
...
-- 11. qstore_plan_variance (multi-plan queries в last 1h on @db; sp_executesql + OUTPUT trick)
...
BEGIN
    ...
    SELECT @json_out = (
        ...
        JOIN ' + QUOTENAME(@db) + N'.sys.query_store_plan qsp ON qsq.query_id = qsp.query_id
        JOIN ' + QUOTENAME(@db) + N'.sys.query_store_runtime_stats qsrs ON qsp.plan_id = qsrs.plan_id
        WHERE qsrs.last_execution_time > DATEADD(HOUR, -1, GETUTCDATE())
          AND qsq.query_id IN (
              SELECT qsq2.query_id
              FROM ' + QUOTENAME(@db) + N'.sys.query_store_query qsq2
              JOIN ' + QUOTENAME(@db) + N'.sys.query_store_plan qsp2 ON qsq2.query_id = qsp2.query_id
              GROUP BY qsq2.query_id HAVING COUNT(DISTINCT qsp2.plan_id) > 1
          )
        ORDER BY qsq.query_id, qsrs.avg_duration DESC
        FOR JSON PATH
    );';
    EXEC sp_executesql @vsql, N'@json_out NVARCHAR(MAX) OUTPUT', @json_out = @plan_variance OUTPUT;
END;

-- Финальная строка — один JSON-объект с 11 ключами (без массива-обёртки).
SELECT (
    SELECT
        ...
        JSON_QUERY(ISNULL(@deadlocks,            N'[]')) AS deadlocks,
        JSON_QUERY(ISNULL(@plan_cache_top_cpu,   N'[]')) AS plan_cache_top_cpu,
        JSON_QUERY(ISNULL(@plan_cache_top_reads, N'[]')) AS plan_cache_top_reads,
        JSON_QUERY(ISNULL(@plan_variance,        N'[]')) AS qstore_plan_variance
