...
try {
    ...
    if (Test-Path $tempErr) {
        $errText = Get-Content -Path $tempErr -Raw -Encoding UTF8
        if ($errText -and $errText.Trim()) {
            Write-Log WARN "sqlcmd stderr: $($errText.Trim())"
        }
    }

    if ($exitCode -ne 0) {
        Write-Log ERROR "sqlcmd exit=$exitCode (см. stderr выше)"
        return
    }

    $rawAll = Get-Content -Path $tempOut -Raw -Encoding UTF8
    if ([string]::IsNullOrWhiteSpace($rawAll)) {
        Write-Log ERROR "sqlcmd вернул пустой вывод"
        return
    }
