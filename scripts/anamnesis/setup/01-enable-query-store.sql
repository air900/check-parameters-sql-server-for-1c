...
SELECT
    name AS [database],
    is_query_store_on AS qs_enabled,
    (SELECT actual_state_desc FROM sys.database_query_store_options
     WHERE database_id = d.database_id) AS qs_state,
    (SELECT max_storage_size_mb FROM sys.database_query_store_options
     WHERE database_id = d.database_id) AS qs_max_size_mb
FROM sys.databases d
WHERE d.name = @db;

PRINT N'Query Store включён на ' + @db;
