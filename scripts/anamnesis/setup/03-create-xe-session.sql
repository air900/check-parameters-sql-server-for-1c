...
-- Сводка через PRINT: проверяем, что session создалась И запущена.
-- (Сложные SELECT по DMV отказались — в server_event_session_targets колонка
-- называется 'name', а в dm_xe_session_targets — 'target_name'. Лишний риск.)
DECLARE @running BIT = CASE
    WHEN EXISTS (SELECT 1 FROM sys.dm_xe_sessions WHERE name = @session_name)
    THEN 1 ELSE 0 END;
DECLARE @defined BIT = CASE
    WHEN EXISTS (SELECT 1 FROM sys.server_event_sessions WHERE name = @session_name)
    THEN 1 ELSE 0 END;
IF @defined = 1 AND @running = 1
    PRINT N'XE session ' + @session_name + N' создана и запущена.';
ELSE IF @defined = 1 AND @running = 0
    PRINT N'XE session ' + @session_name + N' создана, но НЕ запущена (ошибка ALTER...START).';
ELSE
    PRINT N'ОШИБКА: XE session ' + @session_name + N' не создана.';
