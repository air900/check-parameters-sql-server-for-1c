...
function Start-WatcherInteractive {
    ...
    # Окно показывает: статус scheduled task, число собранных snap-*.json,
    # размер папки snapshots. Обновляется каждые 10 секунд.
...
while (`$true) {
    ...
    Write-Host ''
    `$task = Get-ScheduledTask -TaskName '_DiagEshnWatcher'
    if (`$task) {
    ...
    `$count = `$snaps.Count
    `$sizeMB = if (`$count -gt 0) { ([math]::Round((`$snaps | Measure-Object -Property Length -Sum).Sum / 1MB, 1)) } else { 0 }
    ...
    Write-Host ''
    Write-Host 'Закрыть окно: Ctrl+C или просто закрыть. На основной поток сбора это не повлияет.' -ForegroundColor DarkGray
