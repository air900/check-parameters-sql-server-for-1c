...
function Invoke-SetupSqlServer {
    ...
    foreach ($f in $files) {
        ...
        #
        # Кодировка input: setup-скрипты сохранены как UTF-8 BOM, но sqlcmd
        # 13/14/15 на русском Windows нестабильно распознаёт UTF-8 BOM и читает
        # файл как cp1251 → ASCII-идентификаторы ломаются (column 'name' →
        # 'пате'). Перепаковываем в UTF-16 LE BOM (sqlcmd распознаёт BOM
        # UTF-16 однозначно на любой версии/локали). См. bd-wox.
        ...
        try {
            ...
            $sqlText = [System.Text.Encoding]::UTF8.GetString($bytes, $offset, $bytes.Length - $offset)
            [System.IO.File]::WriteAllText($tmpIn, $sqlText, [System.Text.Encoding]::Unicode)
            if ($f -eq '02-set-blocked-process-threshold.sql') {
                & sqlcmd.exe -S "$server" -i $tmpIn -b
            } else {
                & sqlcmd.exe -S "$server" -i $tmpIn -v "db=$db" -b
            }
        }
        finally {
...
function Get-ServerSetupStatus {
    ...
    # См. комментарий выше про UTF-16 LE BOM для надёжной передачи в sqlcmd.
    $tmpIn = [System.IO.Path]::GetTempFileName()
    try {
        $bytes = [System.IO.File]::ReadAllBytes($checkScript)
        $offset = 0
        if ($bytes.Length -ge 3 -and $bytes[0] -eq 0xEF -and $bytes[1] -eq 0xBB -and $bytes[2] -eq 0xBF) {
            $offset = 3
        }
        $sqlText = [System.Text.Encoding]::UTF8.GetString($bytes, $offset, $bytes.Length - $offset)
        [System.IO.File]::WriteAllText($tmpIn, $sqlText, [System.Text.Encoding]::Unicode)
        $output = & sqlcmd.exe -S "$ServerInstance" -d master -i $tmpIn -b -h-1 2>&1
