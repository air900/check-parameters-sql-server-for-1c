...
function Invoke-SetupSqlServer {
    ...
    Write-Host "Будут выполнены четыре SQL-скрипта:"
    ...
    Write-Host "  04-grant-watcher-permissions       (VIEW SERVER STATE для NT AUTHORITY\SYSTEM)"
    Write-Host ""
    ...
    # 02-set-blocked-process-threshold не использует -v db=...
    # 01, 03, 04 нуждаются в -v db=... для USE QUOTENAME(@db) или per-DB операций.
    $files = @(
        @{ Name = '01-enable-query-store.sql';            NeedsDb = $true  },
        @{ Name = '02-set-blocked-process-threshold.sql'; NeedsDb = $false },
        @{ Name = '03-create-xe-session.sql';             NeedsDb = $true  },
        @{ Name = '04-grant-watcher-permissions.sql';     NeedsDb = $true  }
    )
...
function Test-SnapshotOnce {
    ...
    if ($produced) {
        Write-Host "Готово: создан $($produced.Name) ($($produced.Length) байт)." -ForegroundColor Green
    } else {
