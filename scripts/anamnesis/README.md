...
Полный индекс документации (active + archived spec/plan) — [`MANIFEST.md`](MANIFEST.md). Манифест поддерживается через `/delegate-with-context` Phase 8 doc-curator: при выполнении задачи в зоне `scripts/anamnesis/` skill автоматически предлагает добавить новые result-артефакты, заархивировать superseded specs/plans и зачистить устаревшие записи.
