...
## Главная точка входа: интерактивное меню

Всё-в-одном — настройка, запуск, остановка, отправка архива:

```powershell
C:\Anamnesis\Run-Anamnesis.ps1
```

Меню само спросит SQL Server и БД, выполнит нужные команды,
запустит watcher в отдельном окне с live-прогрессом.

`install-anamnesis.ps1` запускает это меню автоматически после установки.

## За 5 шагов вручную (если меню не подходит)
